function mat_D_plus_x = assemble_D_plus_2d_x(N_x, N_y, reference_gradient_x)
    n_basis = 3; % 基函数数量为3，对应基函数为1, x, y
    N_tot = N_x * N_y * n_basis;
    mat_D_plus_x = sparse(N_tot, N_tot);
        ...
        for j_row = 1:N_y
            for k_row = 0:(n_basis - 1)
                ...
                for k_col = 0:(n_basis - 1)
                    ...
                    mat_D_plus_x(ind_row, ind_col) = -reference_gradient_x(k_row + 1, k_col + 1);
                    % flux for the left edge
                    mat_D_plus_x(ind_row, ind_col) = mat_D_plus_x(ind_row, ind_col) ...
                                                     - flux_left_edge_plus(k_row + 1, k_col + 1);
                    % flux for the right edge
                    if i_row < N_x
                        ind_col = index_map_space_2d(i_row + 1, j_row, k_col, N_y);
                        mat_D_plus_x(ind_row, ind_col) = mat_D_plus_x(ind_row, ind_col) ...
                                                         + flux_right_edge_plus(k_row + 1, k_col + 1);
                    end
                end % k_col
            end % k_row
        end % j_row
